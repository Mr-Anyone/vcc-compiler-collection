...
TokenType Tokenizer::getKeyword(const std::string& keyword){
    ...
    assert(TokenType::KeywordStart < result 
            && TokenType::KeywordEnd > result 
...
void Token::dump() const {
    switch(type){
        ...
        case IntegerLiteral: 
            ...
            break;
        case Invalid: 
            std::cout <<"token (invalid): " << "invalid" << std::endl;
            break;
...
Token::Token(TokenType type): type(type){
    assert((type == LeftParentheses || type == RightParentheses || type == EndOfFile || (type > KeywordStart && type < KeywordEnd))
            && "it must be parenthesis type style or keyword!");
}

Token::Token(std::string&& string): 
    type(Identifier), string_literal(string) {
}
