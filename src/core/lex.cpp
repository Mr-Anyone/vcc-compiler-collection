...
Token Tokenizer::readOneToken()
{
    ...
    while (!m_file.eof())
    {
        ...
        // if we see either of these, it means that we should stop reading
        // token because it doesn't make sense for the same token to be
        // joined by ' ' or
        // \n
        if (c == ' ' || c == '\n' || c == 0)
        {
...
vcc::FilePos Token::getPos() const
{
    return pos;
}
