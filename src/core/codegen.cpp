...
void CodeGenerator::emitStatement(Statement *stmt, ContextHolder holder) {
  ...
  case code::FunctionArgLists:
    return emitFunctionArgListsStatement(dyncast<FunctionArgLists>(stmt),
                                         holder);
...
llvm::Value *CodeGenerator::emitExpression(Expression *expr,
  ...
  default:
    assert(false &&
           "unhandled expression kind in CodeGenerator::emitExpression");
    return nullptr;
  }
}
...
llvm::Value *CodeGenerator::emitRefExpression(LocatorExpression *expr,
  ...
  case code::MemberAccessExpression:
    return emitRefMemberAccessExpression(
        dyncast<MemberAccessExpression>(expr), holder);
  case code::ArrayAccessExpression:
  ...
  default:
    assert(false &&
           "unhandled locator expression in CodeGenerator::emitRefExpression");
    return nullptr;
  }
}
...
void CodeGenerator::emitAssignmentStatement(AssignmentStatement *stmt,
                                            ContextHolder holder) {
  assert(isa<LocatorExpression>(stmt->m_ref_expr) &&
...
void CodeGenerator::emitIfStatement(IfStatement *stmt, ContextHolder holder) {
  llvm::Function *function =
      stmt->getFirstFunctionDecl()->getLLVMFunction();
  ...
  ASTBase *last_expression =
      stmt->m_statements[stmt->m_statements.size() - 1];
...
void CodeGenerator::emitExternalDeclStatement(FunctionDecl *decl,
  ...
  decl->m_function = llvm::Function::Create(
      function_type, llvm::Function::ExternalLinkage, decl->m_name,
      holder->module);
}
...
llvm::Value *CodeGenerator::emitIntegerBinaryExpression(
    BinaryExpression *expr, ContextHolder holder, llvm::Value *left_hand_side,
...
llvm::Value *CodeGenerator::emitBinaryExpression(BinaryExpression *expr,
  ...
  if (right_hand_side->getType()->isIntegerTy() &&
      left_hand_side->getType()->isIntegerTy())
    return emitIntegerBinaryExpression(expr, holder, left_hand_side,
                                       right_hand_side);
...
llvm::Value *CodeGenerator::emitIdentifierExpression(IdentifierExpr *expr,
  ...
  return holder->builder.CreateLoad(expr->getType(holder)->getType(holder),
                                    loc_value);
}

llvm::Value *CodeGenerator::emitStartOfPointerFromParentExpression(
    Expression *expression, ContextHolder holder) {
  ...
  return emitCurrentRefArrayAccessExpression(
      dyncast<ArrayAccessExpression>(expression), holder);
}
...
llvm::Value *CodeGenerator::emitCurrentRefMemberAccessExpression(
  ...
  llvm::Value *start_of_pointer =
      (expr->m_parent == nullptr)
          ? holder->symbol_table.lookupLocalVariable(expr, expr->m_base_name)
                .value
...
llvm::Value *CodeGenerator::emitCurrentRefArrayAccessExpression(
  ...
  llvm::Value *start_of_pointer =
      (expr->m_parent_expression == nullptr)
          ? holder->symbol_table.lookupLocalVariable(expr, expr->m_base_name)
                .value
          : emitStartOfPointerFromParentExpression(expr->m_parent_expression,
                                                    holder);
...
llvm::Value *
CodeGenerator::emitCurrentRefDeRefExpression(DeRefExpression *expr,
                                             ContextHolder holder) {
...
llvm::Value *
CodeGenerator::emitMemberAccessExpression(MemberAccessExpression *expr,
                                          ContextHolder holder) {
...
llvm::Value *
CodeGenerator::emitArrayAccessExpression(ArrayAccessExpression *expr,
                                         ContextHolder holder) {
  if (expr->m_child_posfix_expression)
    return emitExpression(expr->m_child_posfix_expression, holder);

  llvm::Value *start_of_pointer =
      emitCurrentRefArrayAccessExpression(expr, holder);
  Type *current_type = expr->getGEPType(holder);
  Type *child_type =
...
llvm::Value *CodeGenerator::emitDeRefExpression(DeRefExpression *expr,
  ...
  llvm::Type *base_type =
      expr->m_ref->getType(holder)->getAs<PointerType>()->getPointee()->getType(
          holder);
...
llvm::Value *CodeGenerator::emitBuiltinCastExpression(CastExpression *expr,
  ...
  if (from->isIntegerKind() && to->isFloat()) {
    llvm::Value *value =
        emitExpression(expr->m_to_be_casted_expression, holder);
...
llvm::Value *CodeGenerator::emitCastExpression(CastExpression *expr,
  ...
  if (from_type->isBuiltin() && expr->m_cast_to->isBuiltin())
    return emitBuiltinCastExpression(expr, from_type->getAs<BuiltinType>(),
                                     expr->m_cast_to->getAs<BuiltinType>(),
                                     holder);
...
llvm::Value *
CodeGenerator::emitRefIdentifierExpression(IdentifierExpr *expr,
                                           ContextHolder holder) {
  return holder->symbol_table.lookupLocalVariable(expr, expr->m_name).value;
}

llvm::Value *CodeGenerator::emitRefMemberAccessExpression(
    MemberAccessExpression *expr, ContextHolder holder) {
  if (expr->m_child_posfix_expression)
...
llvm::Value *CodeGenerator::emitRefArrayAccessExpression(
    ArrayAccessExpression *expr, ContextHolder holder) {
  if (expr->m_child_posfix_expression)
    return emitRefExpression(expr->m_child_posfix_expression, holder);
